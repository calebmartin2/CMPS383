...
export function EditProduct({ product, handleClose }) {
    ...
    const [showIconUnchanged, setShowIconUnchaged] = useState(true);
    const [showPicturesUnchanged, setShowPicturesUnchanged] = useState(true);

    function handleIconUnchangedShow() {
        iconRef.current && iconRef.current.files.length === 0 ? setShowIconUnchaged(true) : setShowIconUnchaged(false)
    }

    function handlePicturesUnchangedShow() {
        pictureRef.current && pictureRef.current.files.length === 0 ? setShowPicturesUnchanged(true) : setShowPicturesUnchanged(false)
    }
            ...
            <Modal.Body>
                <Form onSubmit={handleEdit}>
                    ...
                    <Form.Group className="mb-4">
                        <Form.Label>Icon (must be 1:1, max size 100KiB) {showIconUnchanged && "(unchanged)"}</Form.Label>
                        <Form.Control type="file" accept="image/png, image/jpeg, image/webp" ref={iconRef} onInput={handleIconUnchangedShow}></Form.Control>
                    </Form.Group>
                    <Form.Group className="mb-4">
                        <Form.Label>Pictures (must be 16:9, max size 5MiB) {showPicturesUnchanged && "(unchanged)"}</Form.Label>
                        <Form.Control type="file" accept="image/png, image/jpeg, image/webp" ref={pictureRef} multiple onInput={handlePicturesUnchangedShow}></Form.Control>
                    </Form.Group>
                    <Modal.Footer>
